...
data_folder=data     # Directory to save downloaded data
tag=respin           # Suffix for job name.
use_dial_file=false  # Whether to use utt2dial file for dialect-wise scoring
tokenizer_path=      # Path to the tokenizer file
loss_scale_ce=1.0    # Loss scale for cross entropy loss
...
Options:
    ...
    --data_folder    # Directory to save downloaded data (default="${data_folder}").
    --tag            # Suffix for job name (default="${tag}").
    --use_dial_file  # Whether to use utt2dial file for dialect-wise scoring (default="${use_dial_file}").
...
if [ "${lang}" != noinfo ]; then
    token_listdir=${data_folder}/${lang}_token_list
else
    token_listdir=${data_folder}/token_list
fi
...
chartoken_list_did="${token_listdir}"/char/tokens_did.txt
...
if [ "${token_type}" = bpe ]; then
    ...
    token_list_did="${chartoken_list_did}"
    bpemodel=none
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ] && ! [[ " ${skip_stages} " =~ [[:space:]]1[[:space:]] ]]; then
    log "Stage 1: Data preparation for ${data_folder}/${train_set}, ${data_folder}/${valid_set}, etc."
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ] && ! [[ " ${skip_stages} " =~ [[:space:]]2[[:space:]] ]]; then
    if [ -n "${speed_perturb_factors}" ]; then
       log "Stage 2: Speed perturbation: ${data_folder}/${train_set} -> ${data_folder}/${train_set}_sp"
       for factor in ${speed_perturb_factors}; do
           if python3 -c "assert ${factor} != 1.0" 2>/dev/null; then
               scripts/utils/perturb_data_dir_speed.sh \
                   ...
                   "${factor}" "${data_folder}/${train_set}" "${data_folder}/${train_set}_sp${factor}"
               _dirs+="${data_folder}/${train_set}_sp${factor} "
           else
               # If speed factor is 1, same as the original
               _dirs+="${data_folder}/${train_set} "
           fi
        done
        utils/combine_data.sh \
            ${ref_text_files_str:+--extra_files "${ref_text_files_str}"} \
            "${data_folder}/${train_set}_sp" ${_dirs}
    else
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    ...
    if [ "${feats_type}" = raw ]; then
        log "Stage 3: Format wav.scp: ${data_folder}/ -> ${data_feats}"
        ...
        for dset in ${_dsets}; do
            ...
            utils/copy_data_dir.sh --validate_opts --non-print ${data_folder}/"${dset}" "${data_feats}${_suf}/${dset}"
            ...
            if [ ${#ref_text_files[@]} -gt 1 ]; then
                ...
                for ref_txt in ${ref_text_files[@]}; do
                    [ -f ${data_folder}/${dset}/${ref_txt} ] && cp ${data_folder}/${dset}/${ref_txt} ${data_feats}${_suf}/${dset}
                done
            fi

            _opts=
            if [ -e ${data_folder}/"${dset}"/segments ]; then
                # "segments" is used for splitting wav files which are written in "wav".scp
                # into utterances. The file format of segments:
                #   <segment_id> <record_id> <start_time> <end_time>
                #   "e.g. call-861225-A-0050-0065 call-861225-A 5.0 6.5"
                # Where the time is written in seconds.
                _opts+="--segments ${data_folder}/${dset}/segments "
            fi
            ...
            scripts/audio/format_wav_scp.sh --nj "${nj}" --cmd "${train_cmd}" \
                ...
                "${data_folder}/${dset}/wav.scp" "${data_feats}${_suf}/${dset}"
        ...
        for dset in ${_dsets}; do
            if [ -e "${data_folder}/${dset}/segments" ]; then
                log "Error: ${data_folder}/${dset}/segments is existing. Please use --feats_type raw"
                exit 1
            fi
            if [ "${dset}" = "${train_set}" ] || [ "${dset}" = "${valid_set}" ]; then
                _suf="/org"
            ...
            if [ "${dset}" = "${train_set}" ] || [ "${dset}" = "${valid_set}" ]; then
                ...
                if [ -e "${data_folder}/${dset}/utt2dur" ]; then
                    _fs=$(python3 -c "import humanfriendly as h;print(h.parse_size('${fs}'))")
                    <${data_folder}/${dset}/utt2dur awk '{ print $1, int($2*'${_fs}'); }' > "${data_feats}${_suf}/${dset}"/utt2num_samples

                elif [ -e "${data_folder}/${dset}/utt2num_samples" ]; then
                    cp "${data_folder}/${dset}/utt2num_samples" "${data_feats}${_suf}/${dset}"/utt2num_samples

                else
                    log "Error: ${data_folder}/${dset}/utt2dur or ${data_folder}/${dset}/utt2num_samples must be existing for train_set and valid_set. Please use --feats_type raw. If you'd like to perform this script for evaluation, please give --skip_train true"
                    exit 1
                fi
            fi
        ...
        log "[Require Kaldi] Stage 3: ${feats_type} extract: ${data_folder}/ -> ${data_feats}"
        ...
        log "Stage 3: ${feats_type} extract: ${data_folder}/ -> ${data_feats}"
        ...
        for dset in ${_dsets}; do
            ...
            if [ ! -f ${data_folder}/"${dset}"/wav.scp ]; then
                if [ ! -f ${data_folder}/"${dset}"/segments ]; then
                    <${data_folder}/"${dset}"/feats.scp awk ' { print($1,"<DUMMY>") }' > ${data_folder}/"${dset}"/wav.scp
                else
                    <${data_folder}/"${dset}"/segments awk ' { print($2,"<DUMMY>") }' > ${data_folder}/"${dset}"/wav.scp
                fi
            fi
            utils/copy_data_dir.sh --validate_opts --non-print ${data_folder}/"${dset}" "${data_feats}${_suf}/${dset}"
...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ] && ! [[ " ${skip_stages} " =~ [[:space:]]11[[:space:]] ]]; then
    ...
    # shellcheck disable=SC2086
    ${python} -m espnet2.bin.launch_debug \
        --cmd "${cuda_cmd} --name ${jobname}" \
...
if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ] && ! [[ " ${skip_stages} " =~ [[:space:]]13[[:space:]] ]]; then
    ...
    for dset in ${_dsets}; do
        ...
        [ -f local/score_lang_id_jan21.py ] && python local/score_lang_id_jan21.py --ref_file ${_data}/text --hyp_file ${_dir}/text --out ${_dir}/lid_score.txt --mismatch_file ${_dir}/lid_mismatch.tsv --lang_file ${_dir}/lang_id_refs.tsv
        if [ $use_dial_file = true ]; then
            [ -f local/score_cer_wer.sh ] && local/score_cer_wer.sh --ref ${_data}/text --hyp ${_dir}/text --wdir ${_dir} --nlsyms_txt ${data_folder}/list_lang --dial_file ${data_folder}/${dset}/utt2dial --use_dial_file true
        else
            [ -f local/score_cer_wer.sh ] && local/score_cer_wer.sh --ref ${_data}/text --hyp ${_dir}/text --wdir ${_dir} --nlsyms_txt ${data_folder}/list_lang
        fi
