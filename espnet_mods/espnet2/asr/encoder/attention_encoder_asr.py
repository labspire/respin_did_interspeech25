...
class CombinedEncoder(nn.Module):
    def __init__(self, output_dim=256, feature_dim1=128, feature_dim2=64, num_heads=8, hidden_dim=512, 
        ...
        """
        Fully Simplified Combined Encoder for Dialect Identification.
        ...
        self.attention_encoder = AttentionEncoder(
            feature_dim=feature_dim1 + feature_dim2,  # Combined feature dimension
            num_heads=num_heads, 
        ...
        self.norm = nn.LayerNorm(feature_dim1 + feature_dim2)
        self.linear = nn.Sequential(
            nn.Linear(feature_dim1 + feature_dim2, output_dim),
            nn.GELU(),  # Activation
    ...
    def forward(self, enc_out1, enc_out2, encoder_out_lens, device):
        """
        Forward pass for the simplified encoder.

        Args:
            enc_out1 (torch.Tensor): First input tensor of shape (B, T, feature_dim1).
            enc_out2 (torch.Tensor): Second input tensor of shape (B, T, feature_dim2).
        ...
        # Concatenate enc_out1 and enc_out2
        combined_features = torch.cat((enc_out1, enc_out2), dim=-1)  # (B, T, feature_dim1 + feature_dim2)

        # Pass through the attention encoder
        attention_output = self.attention_encoder(combined_features, mask)
