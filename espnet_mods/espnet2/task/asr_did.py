...
from espnet2.asr.encoder.conformer_encoder import ConformerEncoder # Change this line
...
from espnet2.asr.espnet_model_did import ESPnetASRModel # Change this line
...
from espnet2.train.preprocessor_did import (
    AbsPreprocessor,
    CommonPreprocessor,
...
class ASRTask(AbsTask):
    ...
    def add_task_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            ...
            type=str_or_none,
            default=None,
            help="A text mapping int-id to token for DID",
        )
        group.add_argument(
            "--loss_scale_ce",
            type=float_or_none,
            default=5.0,
            help="Loss scaling factor for CE loss",
        )
        group.add_argument(
            "--tokenizer_path",
            type=str_or_none,
    ...
    def build_model(cls, args: argparse.Namespace) -> ESPnetASRModel:
        if isinstance(args.token_list, str):
            ...
            with open(args.token_list_did, encoding="utf-8") as f:
                token_list_did = [line.rstrip() for line in f]

            # Overwriting token_list to keep it as "portable".
            args.token_list = list(token_list)
            args.token_list_did = list(token_list_did)
        elif isinstance(args.token_list, (tuple, list)):
            token_list = list(args.token_list)
            token_list_did = list(args.token_list_did)
        else:
            raise RuntimeError("token_list must be str or list")
        
        if isinstance(args.loss_scale_ce, float):
            loss_scale_ce = args.loss_scale_ce
        ...
        num_dial = len(token_list_did)
        logging.info(f"Vocabulary size: {vocab_size }")
        logging.info(f"Number of Dialects: {num_dial }")
        logging.info(f"Loss Scale CE: {loss_scale_ce }")
        ...
        ce = CrossEntropyModule(odim=num_dial, encoder_output_size=num_dial)
