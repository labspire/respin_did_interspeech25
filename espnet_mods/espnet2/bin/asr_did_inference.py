...
from espnet2.tasks.asr_did import ASRTask # Change this to the correct task
...
class Speech2Text:
    ...
    def __init__(
        ...
        token_list_did = asr_model.token_list_did
    ...
    def __call__(self, speech: Union[torch.Tensor, np.ndarray]) -> Union[
        ...
        if self.enh_s2t_task or self.multi_asr:
            ...
            # # enc_comb_out = self.enc_comb(bneck_out, enc_did_out, encoder_out_lens, encoder_out.device)
            ...
            # enc_did, _ = self.asr_model.enc_did(enc, enc_olens, enc.device)
            enc_rob = self.asr_model.enc_roberta(enc, enc_olens)
            enc_bneck = self.asr_model.enc_bneck(enc, enc_olens)
            enc2, enc2_embed = self.asr_model.enc_comb(enc_bneck, enc_rob, enc_olens, enc.device)
            enc = self.asr_model.enc_comb_asr(enc, enc2_embed, enc_olens, enc.device)
            ...
            if enc2 is not None:
                encoder_did_res = self._decode_did(list(enc2))
            else:
                encoder_did_res = None
                # if results is already a tuple, append the new results to it
                # if isinstance(results, tuple):
                #     results = (results[0], results[1], encoder_did_res)
                # else:
                #     results = (results, encoder_did_res)
                
            # if both intermediate CTC and DID predictions are available then merge them
            if encoder_interctc_res is not None and encoder_did_res is not None:
                for key in encoder_did_res.keys():
                    encoder_interctc_res[key] = encoder_did_res[key]
                results = (results, encoder_interctc_res)
            elif encoder_did_res is not None:
                results = (results, encoder_did_res)
            elif encoder_interctc_res is not None:
                results = (results, encoder_interctc_res)
    ...
    def _decode_interctc(
        ...
        for layer_idx, encoder_out in intermediate_outs:
            ...
            y = self.asr_model.ce.argmax(encoder_out)[0]
    ...
    @typechecked
    def _decode_did(
        self, intermediate_outs: List[torch.Tensor]
    ) -> Dict[int, List[str]]:
        ...
        for encoder_out in intermediate_outs:
            
            y = self.asr_model.ce.argmax(encoder_out.unsqueeze(0))[0]  # batch_size = 1
            y = [str(y.tolist() + 1)]
            y_softmax = self.asr_model.ce.softmax(encoder_out.unsqueeze(0))[0]
            y_softmax = y_softmax.tolist()
            # y = [token_list[x] for x in y]
            
            # concatenate y and y_softmax, convert to string
            y = [str(y) for y in y] + [str(y) for y in y_softmax]
